...
class _CreateLeadScreenState extends State<CreateLeadScreen> {
  ...
  void _onProjectChanged(String? projectId) {
    ...
    if (project.state != null) {
      ...
      try {
        ...
        _loadCitiesAndLocations(state.id, project.city);
      } catch (e) {
  ...
  Future<void> _loadCitiesAndLocations(
    String stateId,
    String? projectCity,
  ) async {
    try {
      // First load cities for the state
      final cities = await MasterDataService.getCities(stateId: stateId);
      setState(() {
        _cities = cities;
      });

      // Then if project has a city, find it and load locations
      if (projectCity != null) {
        try {
          final city = cities.firstWhere((c) => c.name == projectCity);
          await _loadLocations(city.id);
        } catch (e) {
          print('City not found in loaded cities: $projectCity');
        }
      }
    } catch (e) {
  ...
  Future<void> _saveAndContinue() async {
    // Validate basic form if we're on the first page
    if (_currentPage == 0) {
      ...
      // Validate preference form if we're on the second page
  ...
  Future<void> _saveLead() async {
    ...
    if (_selectedPurchaseYear == null || _selectedPurchaseYear!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select a purchase plan year')),
      );
      return;
    }

    if (_selectedPurchaseMonth == null || _selectedPurchaseMonth!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select a purchase plan month')),
      );
      return;
    }

    if (_selectedBudget == null || _selectedBudget!.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Please select a budget')));
      return;
    }
    ...
    try {
      ...
      if (_selectedProject != null) {
        try {
          final project = _projects.firstWhere((p) => p.id == _selectedProject);
          projectName = project.name;
        } catch (e) {
          print('Project not found: $_selectedProject');
          projectName = 'Unknown Project';
        }
      }

      // Get assigned user name safely
      String assignedToName = '';
      if (_selectedAssignTo != null && _selectedAssignTo!.isNotEmpty) {
        try {
          final user = _users.firstWhere((u) => u.id == _selectedAssignTo);
          assignedToName = user.name;
        } catch (e) {
          print('User not found: $_selectedAssignTo');
